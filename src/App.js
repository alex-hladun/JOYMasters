...
function App() {
  ...
  useEffect(() => {
    const leaderboardData = axios.get(`https://api.sportsdata.io/golf/v2/json/Leaderboard/375?key=${key}`).then((res, err) => {
  ...
  if (leaderboard['1'] && players[0] && teamArray[0] && teamArray[0].total) {
    playerHTML = players.map((player, index) => {

      // teamTotal[teams[player.PlayerID]]
    ...
    leaderboardHTML = teamArray.map((team, index) => {
